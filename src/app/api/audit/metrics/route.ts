...
export async function GET() {
  ...
  try {
    ...
    const recentHits = await db
      .select()
      .from(apiMetrics)
      .where(and(eq(apiMetrics.userId, userId), gte(apiMetrics.createdAt, oneHourAgo)))
    ...
    const stats24h = await db
      ...
      .from(apiMetrics)
      .where(and(eq(apiMetrics.userId, userId), gte(apiMetrics.createdAt, twentyFourHoursAgo)));
