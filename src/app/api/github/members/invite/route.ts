...
import { createGitHubHeaders, GITHUB_API_BASE } from '@/lib/github/client';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (isEmail) {
      ...
      if (!userRes.ok) {
        return NextResponse.json(
          { error: { message: `User ${invitee} not found` } },
          { status: 404 },
        );
      }
    ...
    return NextResponse.json({ error: { message: 'Failed to invite member' } }, { status: 500 });
  }
}
