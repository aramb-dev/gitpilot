    ...
    global.fetch = mock((url) => {
      if (url === 'https://api.github.com/user/orgs?per_page=100') {
        return Promise.resolve(
          new Response(
            JSON.stringify([{ id: 1, login: 'org1', avatar_url: 'url1', description: 'desc1' }]),
            { status: 200 },
          ),
        );
      }
      if (url === 'https://api.github.com/user') {
        return Promise.resolve(
          new Response(
            JSON.stringify({ id: 999, login: 'user1', avatar_url: 'user_url', bio: 'user bio' }),
            { status: 200 },
          ),
        );
      }
