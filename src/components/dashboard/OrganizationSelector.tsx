    ...
    <div className="bg-[#0d0d0d] border border-[#333] p-6 font-mono">
      ...
      <div className="p-4 bg-[#00ff00]/5 border border-[#00ff00]/30 mb-6">
        ...
        <p className="text-xs text-[#666] mb-3">
          some organizations require explicit{' '}
          <span className="text-white">Third-party application access</span> via GitHub settings.
        </p>
        <button
      ...
      <div className="space-y-2 mb-6">
        {organizations.map((org) => (
          <div
            key={org.id}
            className="flex items-center space-x-3 p-3 hover:bg-[#1a1a1a] transition-colors"
          >
            <Checkbox
              id={`org-${org.id}`}
            ...
            >
              {org.avatar_url ? (
                <img
                  src={org.avatar_url}
                  alt={org.login}
                  className="w-8 h-8 border border-[#333] flex-shrink-0"
                />
              ) : (
              ...
              <div>
                ...
                {org.description && <div className="text-[#666] text-xs">{org.description}</div>}
              </div>
            </label>
          </div>
        ))}
      ...
      <div className="pt-4 flex items-center justify-end space-x-4 border-t border-[#333]">
        {success && <span className="text-[#00ff00] text-sm font-medium">saved successfully</span>}
        <Button
          onClick={handleSave}
          disabled={saving}
