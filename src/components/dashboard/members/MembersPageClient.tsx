...
import { UserPlus, RefreshCw, AlertCircle, X, Search, Trash2, Shield } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          >
            ...
            {!preferences?.selectedOrgs?.length && (
              <option value="">no_orgs_selected</option>
            )}
          </select>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="border border-[#333] bg-[#0d0d0d]">
              <div className="p-4 border-b border-[#333] bg-[#1a1a1a] flex items-center justify-between">
                <span className="text-[10px] text-[#666] uppercase font-bold tracking-widest">organization_members</span>
                <span className="text-[10px] text-[#666]">{filteredMembers.length} active</span>
              </div>
              
              <div className="divide-y divide-[#333]">
                {filteredMembers.map((member) => (
                  <div key={member.id} className="p-4 flex items-center gap-4 hover:bg-[#151515] transition-colors group">
                    <input
                      type="checkbox"
                      checked={selectedMembers.includes(member.login)}
                      onChange={(e) => {
                        if (e.target.checked) setSelectedMembers([...selectedMembers, member.login]);
                        else setSelectedMembers(selectedMembers.filter(m => m !== member.login));
                      }}
                      className="w-4 h-4 accent-[#00ff00]"
                    />
                    <img src={member.avatarUrl} className="w-8 h-8 rounded-none border border-[#333]" />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-white">{member.login}</span>
                        {member.role === 'admin' && (
                          <Badge variant="outline" className="text-[9px] border-purple-500/30 text-purple-400 font-mono py-0">ADMIN</Badge>
                        )}
                      </div>
          ...
          <div className="space-y-6">
            <div className="border border-[#333] bg-[#0d0d0d]">
              <div className="p-4 border-b border-[#333] bg-[#1a1a1a]">
                <span className="text-[10px] text-[#666] uppercase font-bold tracking-widest">Pending Invitations</span>
              </div>
              <div className="p-4 space-y-4">
                {invitations.length === 0 ? (
                  <p className="text-[10px] text-[#444] text-center py-4 italic">no_pending_invites</p>
                ) : (
                  invitations.map(invite => (
                    <div key={invite.id} className="text-xs space-y-1 p-2 bg-[#151515] border border-[#333]">
                      ...
                      <div className="flex justify-between items-end">
                        ...
                        <button className="text-[9px] text-red-900 hover:text-red-500 uppercase font-bold">cancel</button>
                      </div>
                    </div>
                  ))
                )}
            ...
            <div className="p-4 bg-[#00ff00]/5 border border-[#00ff00]/10 text-[10px] text-[#666] leading-relaxed">
              ...
              <p>&gt; only organization owners can manage member roles and invitations. permissions are synchronized with github.</p>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-xl px-4 z-40">
          <div className="bg-[#0d0d0d] border border-[#00ff00]/30 shadow-2xl p-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <span className="bg-[#00ff00] text-black px-2 py-0.5 text-xs font-bold">{selectedMembers.length}</span>
            ...
            <div className="flex items-center gap-2">
              ...
              <X className="w-4 h-4 text-[#444] cursor-pointer ml-2" onClick={() => setSelectedMembers([])} />
            </div>
          </div>
        </div>
      )}
