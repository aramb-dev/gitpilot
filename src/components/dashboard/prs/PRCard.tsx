import { ExternalLink, GitCompare, GitPullRequest, MessageSquare } from 'lucide-react';
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-start gap-3 flex-1">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-sm text-white mt-0.5 break-words">{pr.title}</div>
          </div>
        </div>
      </div>

      {/* Description */}
      {pr.body && <div className="text-xs text-[#888] mb-3 line-clamp-2">{pr.body}</div>}

      {/* Status badges */}
      <div className="flex flex-wrap gap-2 mb-3">
        <Badge variant="outline" className={`${getStateColor()} text-xs`}>
          {getStateLabel()}
        </Badge>

        {pr.draft && (
          <Badge variant="secondary" className="bg-[#666]/20 text-[#888] border-[#666]/30 text-xs">
            draft
          </Badge>
        )}
