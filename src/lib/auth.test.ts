...
describe("authOptions", () => {
  ...
  describe("callbacks", () => {
    it("should assign accessToken to token in jwt callback", async () => {
      const token = {};
      const account = { access_token: "test-token", provider: "github", type: "oauth", providerAccountId: "123" } as any;
      
      const result: any = await authOptions.callbacks?.jwt?.({ token, account, user: null as any, profile: undefined, trigger: "signIn", session: null });
    ...
    it("should persist accessToken in session callback", async () => {
      ...
      const result: any = await authOptions.callbacks?.session?.({ session, token, user: null as any, newSession: null, trigger: "update" });
